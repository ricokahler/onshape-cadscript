...
3. If Chrome reports no extension, run `npx -y onshape-cadscript@0.1.5 setup chrome`. Ask the user to enable Developer Mode, click **Load unpacked**, and select the directory printed by the command. Do not operate Chrome's extension installation controls for them.
...
```sh
npx -y onshape-cadscript@0.1.5 init ./my-print
```
